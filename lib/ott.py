...
def get_supported_providers():
    """Returns a list of supported OTT providers."""
...
def _provider_from_cmd(cmd: str):
    ...
    return _CMD_TO_PROVIDER.get(cmd) or (cmd if cmd in _WORKERS else None)
...
def scrape_ott(service_name: str, target_url: str):
    """
    Scrape OTT information using the specified service.

    Args:
        service_name: The name or alias of the service (e.g., 'netflix', 'primevideo').
    ...
    """
    provider = _provider_from_cmd(service_name)
    if not provider:
