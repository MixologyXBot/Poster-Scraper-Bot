...
def get_supported_services():
    ...
    return list(_BYPASS_ENDPOINTS.keys())

def _bp_srv(name):
    """Normalize service name."""
    name = name.lower().lstrip("/")
    return _BYPASS_CMD_TO_SERVICE.get(name) or (name if name in _BYPASS_ENDPOINTS else None)
...
def bypass_url(service_name: str, target_url: str):
    """
    Bypass a DDL link using the specified service.

    Args:
        service_name: The name or alias of the service (e.g., 'gdflix', 'hubdrive').
        target_url: The URL to bypass.

    Returns:
        tuple: (result_dict, error_message)
               result_dict is None if error occurs.
               error_message is None if successful.
    """
    service = _bp_srv(service_name)
    if not service:
    ...
    try:
        if service == "transfer_it":
            resp = requests.post(api_url, json={"url": target_url}, timeout=20)
        else:
